...
<template>
    <div class="min-h-screen bg-slate-50 font-sans text-slate-800 antialiased">
        ...
        <div class="flex">
            ...
            <div :class="['flex-1 flex flex-col min-h-screen transition-all duration-200', sidebarCollapsed ? 'lg:ml-20' : 'lg:ml-64']">
                ...
                <main class="flex-1 px-4 sm:px-6 py-4 space-y-4 print:hidden">
                    ...
                    <section class="bg-white border border-slate-200 p-4 space-y-3">
                        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                            ...
                            <div class="bg-slate-50 p-3 border border-slate-200 md:w-64 shrink-0 space-y-1.5">
                    ...
                    <div class="bg-white border border-slate-200 overflow-hidden">
                        ...
                        <div class="border-b border-slate-200 bg-slate-50 px-3 pt-1.5">
                            <nav class="flex gap-1.5" role="tablist">
                                ...
                                <button @click="activeTab = 'documents'" :class="['flex items-center gap-1.5 px-4 py-2 text-xs font-semibold rounded-t-md border-t border-x transition-all relative', activeTab === 'documents' ? 'bg-white border-slate-200 text-slate-900 font-bold' : 'border-transparent text-slate-500 hover:text-slate-800 hover:bg-slate-100/60']">
                                    ...
                                    <div v-if="activeTab === 'documents'" class="absolute top-0 left-0 right-0 h-0.5 bg-red-700"></div>
                                </button>
                            </nav>
                        </div>
                        ...
                        <div v-if="activeTab === 'info'" class="p-4 sm:p-5 space-y-5">
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                ...
                                <div class="bg-white border border-slate-200 p-4 space-y-3">
                                    ...
                                    <div class="divide-y divide-slate-100 text-xs">
                                        ...
                                        ]" :key="item.label" class="py-1.5 flex justify-between items-center gap-3 hover:bg-slate-50/60 px-1.5 transition-colors">
                                ...
                                <div class="space-y-4">
                                    <div class="bg-white border border-slate-200 p-4 space-y-3">
                                        ...
                                        <div class="grid grid-cols-2 gap-3">
                                            <div class="p-2.5 bg-slate-50 border border-slate-200 space-y-1">
                                                <span class="text-[10px] font-semibold text-slate-500 uppercase">Status</span>
                                                <div>
                                                    <span :class="[getStatusConfig(project.status).bg, 'inline-flex items-center gap-1 px-2 py-0.5 text-xs font-bold border']">
                                                        <span class="w-1.5 h-1.5 rounded-full" :class="getStatusConfig(project.status).dot"></span>
                                                        {{ project.status }}
                                                    </span>
                                                </div>
                                    ...
                                    <div v-if="project.remarks" class="border border-red-200 bg-red-50/40 p-3 space-y-1 text-xs">
                                    ...
                                    <div v-if="project.description" class="border border-slate-200 bg-slate-50 p-3 space-y-1 text-xs">
                            ...
                            <div class="bg-white border border-slate-200 p-4 space-y-4">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
                                    <div class="border border-slate-200 bg-slate-50/80 p-3 space-y-0.5">
                                    ...
                                    <div class="border border-red-200 bg-red-50/40 p-3 space-y-0.5">
                            ...
                            <div class="bg-white border border-slate-200 p-4 space-y-3">
                                ...
                                <div v-if="!project.technical_preparations || techPrepItems.length === 0" class="text-center py-6 border border-dashed border-slate-200">
                                ...
                                <div v-else class="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                                    <div v-for="item in techPrepItems" :key="item.key" :class="[getTechStatus(item.status).bg, 'border p-2.5 text-center flex flex-col items-center justify-between gap-1 transition-all']">
                                        <div class="flex items-center justify-center h-5 w-5">
                                            ...
                                            <else class="h-4 w-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 12H6" /></else>
                                        </div>
                                        <p class="text-[11px] font-bold text-slate-800 leading-tight">{{ item.label }}</p>
                                        <span class="inline-flex items-center gap-1 px-1.5 py-0.2 text-[10px] font-bold bg-white/90 border border-slate-200">
                            ...
                            <div class="bg-white border border-slate-200 p-4 space-y-3">
                                ...
                                <div v-if="timelineEvents.length === 0" class="text-center py-6 border border-dashed border-slate-200">
                                ...
                                <div v-else class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 text-xs">
                                    <div v-for="event in timelineEvents" :key="event.label" class="p-2.5 bg-slate-50 border border-slate-200 space-y-1">
                        ...
                        <div v-else-if="activeTab === 'documents'" class="p-4 sm:p-5 space-y-4">
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-50 p-3 border border-slate-200">
                                <div class="flex items-center gap-2.5">
                                    <div class="flex h-8 w-8 items-center justify-center bg-red-100 text-red-700">
                                ...
                                <div class="flex flex-wrap items-center gap-2">
                                    ...
                                    <!-- Download All Button -->
                                    <button @click="downloadAllDocs" :disabled="!filteredDocuments.length" class="inline-flex items-center gap-1 px-3 py-1 bg-red-700 hover:bg-red-800 disabled:opacity-50 disabled:cursor-not-allowed text-white text-xs font-bold rounded transition-colors shadow-2xs">
                                        <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
                                        Download All
                                    </button>
                            ...
                            <div v-else-if="docViewMode === 'table'" class="overflow-x-auto border border-slate-200">
        ...
        <div class="hidden print:block p-8 bg-white text-slate-900 font-sans space-y-6">
            ...
            <!-- Report Title Banner -->
            <div class="bg-slate-100 p-4 border border-slate-300">
        ...
        <Transition name="fade">
            <div v-if="previewDoc" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/75 backdrop-blur-xs p-4" @click.self="closePreview">
                <div class="bg-white shadow-xl max-w-4xl w-full h-[85vh] flex flex-col overflow-hidden border border-slate-200">
