...
const selectedProjectStorageKey = 'meo_admin_selected_project_id';
...
// ========== FUND CATEGORIES ==========
const fundCategories = ref({ ...defaultFundCategories });
...
const getFundCategoryLabel = (project) => {
    ...
    if (key) {
        return fundCategories.value[key]?.label || 
               defaultFundCategories[key]?.label || 
...
const fundCategoryOptions = computed(() => {
    // Only use the three default categories: national, provincial, lgu
    const validCategories = ['national', 'provincial', 'lgu'];
    
    return validCategories.map(key => ({
        value: key,
        label: defaultFundCategories[key].label,
    }));
});
...
const openAddModal = async () => {
    resetForm();
    const categories = Object.keys(fundCategories.value);
    if (categories.length > 0) {
        form.fundCategory = categories[0];
    } else {
...
const openEditModal = async (project) => {
    ...
    Object.assign(form, {
        ...
        fundCategory: project.fundCategory || 'national',
...
const fetchFundSources = async (category = null) => {
    ...
    try {
        ...
        const { data } = await window.axios.get(route('admin.projects.fund-sources', params));
        return data.sources || [];
    } catch (error) {
...
const loadAllFundSources = async () => {
    isLoadingSources.value = true;
    try {
        const { data } = await window.axios.get(route('admin.projects.fund-sources'));
        ...
        if (data.sources && Array.isArray(data.sources)) {
            data.sources.forEach(source => {
                if (source.category) {
                    ...
                    groupedSources[source.category].push(source.source);
                }
            });
        }
        ...
        const mergedCategories = { ...defaultFundCategories };
        ...
        Object.keys(groupedSources).forEach(category => {
            ...
            mergedCategories[category].sources = [...new Set(groupedSources[category])].sort();
        });

        fundCategories.value = mergedCategories;
        
    } catch (error) {
        console.error('Error loading fund sources:', error);
        fundCategories.value = { ...defaultFundCategories };
    } finally {
        isLoadingSources.value = false;
    }
};
...
const addProject = async () => {
    ...
    try {
        const endpoint = isEditing.value 
            ? route('admin.projects.update', selectedProject.value.id)
...
const openProject = async (project) => {
    ...
    try {
        const { data } = await window.axios.get(route('admin.projects.show', project.id));
        selectedProject.value = data.project;
...
<template>
    <div class="w-full space-y-4">
        ...
        <template v-else>
            <!-- ===== SIMPLE DASHBOARD ===== -->
            <div class="flex flex-wrap items-center gap-6 sm:gap-8 pb-3 border-b border-gray-100">
                <div class="flex items-baseline gap-2">
                    <span class="text-lg font-bold text-slate-900">{{ dashboardStats.total }}</span>
                    <span class="text-[11px] font-semibold uppercase tracking-wider text-slate-400">Total Projects</span>
                </div>
                <div class="flex items-baseline gap-2">
                    <span class="text-lg font-bold text-blue-600">{{ dashboardStats.ongoing }}</span>
                    <span class="text-[11px] font-semibold uppercase tracking-wider text-slate-400">Ongoing ({{ dashboardStats.avgAccomplishment }}% avg)</span>
                </div>
                <div class="flex items-baseline gap-2">
                    <span class="text-lg font-bold text-emerald-600">{{ dashboardStats.completed }}</span>
                    <span class="text-[11px] font-semibold uppercase tracking-wider text-slate-400">Completed ({{ dashboardStats.total ? Math.round((dashboardStats.completed / dashboardStats.total) * 100) : 0 }}%)</span>
                </div>
            </div>

            <!-- ===== FILTERS (Compact) ===== -->
            <div class="sticky top-3 z-30 bg-white/95 backdrop-blur-sm border border-gray-200 p-3">
            ...
            <!-- ===== PROJECT LIST ===== -->
            <div class="project-list-container bg-white border border-gray-200 overflow-hidden">
                ...
                <div v-else-if="viewMode === 'list'" class="divide-y divide-gray-100">
                    <div
                        ...
                        :key="project.id"
                        class="px-4 py-2.5 hover:bg-slate-50 transition flex items-center justify-between gap-3"
                    >
                        <div class="flex items-center gap-3 flex-1 min-w-0 cursor-pointer" @click="openProject(project)">
                            <!-- Number with dot format based on global index -->
                            <span class="text-xs font-medium text-slate-400 w-6 text-right flex-shrink-0">
                                {{ (currentPage - 1) * perPage + index + 1 }}.
                            </span>
                            
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-2 flex-wrap">
                                    <span class="text-sm font-semibold text-slate-900 truncate">{{ project.name }}</span>
                                    <span :class="[statusClass(project.status), 'px-2 py-0.5 rounded-full text-[10px] font-semibold tracking-wide uppercase']">{{ project.status }}</span>
                                </div>
                                <div class="flex items-center gap-3 mt-0.5 text-xs text-slate-500 flex-wrap">
                                    ...
                                    <span class="flex items-center gap-1 font-medium text-slate-700">
                                        ...
                                        {{ formatCompactCurrency(project.totalCost) }}
                                    </span>
                                    <span class="flex items-center gap-1">
                                        <span class="w-1.5 h-1.5 rounded-full" :class="getCategoryColor(getFundCategory(project)) === 'blue' ? 'bg-blue-500' : getCategoryColor(getFundCategory(project)) === 'green' ? 'bg-green-500' : getCategoryColor(getFundCategory(project)) === 'purple' ? 'bg-purple-500' : 'bg-gray-500'"></span>
                                        {{ getFundCategoryLabel(project) }}
                                    </span>
                                    <span class="flex items-center gap-1 text-slate-700">
                                        <div class="w-12 h-1.5 bg-slate-200 rounded-full overflow-hidden">
                                            <div class="h-1.5 rounded-full" :class="getAccomplishmentColor(project.accomplishment)" :style="{ width: (project.accomplishment || 0) + '%' }"></div>
                                        </div>
                                        <span class="font-semibold text-[10px]">{{ project.accomplishment || 0 }}%</span>
                                    </span>
                                </div>
                            </div>
                        </div>
                        <div class="flex items-center gap-2">
                            <svg class="h-4 w-4 text-gray-300 flex-shrink-0 cursor-pointer" fill="none" stroke="currentColor" viewBox="0 0 24 24" @click="openProject(project)">
                            ...
                            <div class="relative project-actions-menu">
                                ...
                                <div
                                    v-if="showActionsMenu === project.id"
                                    class="absolute right-0 top-8 z-10 w-36 rounded-lg border border-gray-200 bg-white shadow-lg"
                                >
                                    <button
                                        @click="handleEditFromMenu(project)"
                                        class="block w-full px-4 py-2 text-left text-sm text-gray-700 hover:bg-gray-50 transition"
                                    >
                                        Edit
                                    </button>
                                    <button
                                        @click="handleDeleteFromMenu(project.id)"
                                        class="block w-full rounded-b-lg px-4 py-2 text-left text-sm text-red-700 hover:bg-red-50 transition"
                                    >
                                        Delete
                                    </button>
                                </div>
                ...
                <Transition name="modal">
                    <div v-if="showAddModal" class="fixed inset-0 z-50 overflow-y-auto">
                        ...
                        <div class="flex min-h-full items-center justify-center p-4">
                            <div class="relative w-full max-w-2xl bg-white shadow-2xl border border-gray-200" @click.stop>
                                ...
                                <form @submit.prevent="addProject" class="p-6 max-h-[70vh] overflow-y-auto">
                                    <div class="grid grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700">Project Cost (Php) <span class="text-red-500">*</span></label>
                                            <input 
                                        ...
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700">Date Started <span class="text-red-500">*</span></label>
                                            <input v-model="form.startDate" required type="date" :class="inputClass" />
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700">Original Date of Completion <span class="text-red-500">*</span></label>
                                        ...
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700">Days Due to Time Extension (if any)</label>
                                            <input v-model.number="form.timeExtension" min="0" type="number" :class="inputClass" />
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700">Days Due to Suspension Order (if any)</label>
